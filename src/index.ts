...
program
	...
	.arguments('[filename]')
	.usage(
		`${chalk.magenta('[filename | fullFilename]')} [options] [global options]`
	)
	.description(
		'Cria um novo fonte AdvPL com base nas suas escolhas (comando padrão)'
	)
...
async function runDownload(): Promise<void> {
	...
	let step = logNewSection(
		'Localizando definição de assistentes para transferência.'
	);
	step.start();
	...
	Object.keys(repos).forEach(async (key) => {
		...
		try {
			const repoInfo: RepoInfo | undefined = await getRepoInfo(
				new URL(repo.url),
				repo.path
			);
			...
			if (repoInfo) {
				step.info(
					`Informações obtidas ${repoInfo.branch}:${repoInfo.filePath}`
				);
...
async function runWizard(): Promise<void> {
	...
	let extractWizardStep = logNewSection(
		`Localizando definição de assistentes.`
	);
	...
	try {
		...
		extractWizardStep.succeed(
			'Arquivos de assistentes transferidos e extraídos'
		);
	} catch (e) {
		extractWizardStep.fail(
			'Ocorreu um erro ao transferir e extrair os arquivos de assistentes'
		);
...
function assertValidFilename(filename: string) {
	...
	if (typeof validation === 'string') {
		exitWithError([
			'Não posso criar um arquivo com esse nome:',
			chalk.bold(filename),
			validation,
		]);
	}
...
function exitWithError(args: string | string[], code: number = 1) {
	...
	log.newLine();
	log.nested(
		`Execute ${chalk.green(`${program.name()} --help`)} para mais informações.`
	);
