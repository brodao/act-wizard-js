	...
	if (value === 'example') {
		...
		if (examplesJSON) {
			...
			if (!nameRes.wizardName) {
				console.log();
				console.log(
					'Especifique um assistente ou use o assistente inicial padrão.'
				);
				process.exit(1);
			}
...
export async function resolveWizardArgAsync(
	...
	if (wizard) {
		...
		if (repoUrl) {
			...
			if (!found) {
				oraInstance.fail(
					`Não foi possível localizar o assistente chamado ${chalk.red(
						`"${wizard}"`
					)}.`
				);
				process.exit(1);
...
function hasExample(name: string): Promise<boolean> {
	return isUrlOk(
		`https://api.github.com/repos/expo/examples/contents/${encodeURIComponent(
			name
		)}/package.json`
	);
}
...
function downloadAndExtractRepoAsync(
	...
	return pipeline(
		...
		tar.extract({ cwd: root, strip }, [
			`${name}-${branch}${filePath ? `/${filePath}` : ''}`,
		])
	);
}

function downloadAndExtractExampleAsync(
	root: string,
	name: string
): Promise<void> {
	return pipeline(
...
async function listAsync(): Promise<any> {
	...
	return results.filter(
		({ name, type }: any) => type === 'dir' && !name?.startsWith('.')
	);
}
